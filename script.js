...
async function refreshCountries() {
		...
		let data1;
		try{
			const res1 = await fetch(`/.netlify/functions/countryAPI?code=${countryCode1}`, {cache: "no-store"});
			if(!res1.ok) {
		...
		try{
			const res2 = await fetch(`/.netlify/functions/countryAPI?code=${countryCode2}`, {cache: "no-store"});
